...
interface CityCategoryPageClientProps {
  city: CityData;
...
export function CityCategoryPageClient({
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      console.error("[CityCategoryPage] Error fetching events:", err);
      setError(err instanceof Error ? err.message : "Error al buscar eventos");
      setEvents([]);
    } finally {
      setIsLoading(false);
    }
  }, [filters, city.lat, city.lng, category.slug]);
  ...
  const seoDescription = `Descubre los mejores talleres de ${category.shortName.toLowerCase()} para niños en ${city.name}. ${category.description} En Hazlo con Peques encontrarás actividades de ${category.shortName.toLowerCase()} verificadas y organizadas por profesionales en ${city.name} y alrededores. Reserva online y disfruta de experiencias únicas con tus peques.`;
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative z-10">
            ...
            <Breadcrumbs
              items={[
                ...
                { label: city.name, href: `/eventos/${city.slug}` },
            ...
            >
              ...
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4">
                {category.shortName} para niños en{" "}
                <span className="rainbow-text-animated">{city.name}</span>
              </h1>

              <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                Talleres y actividades de {category.shortName.toLowerCase()} para peques en {city.name} y {city.region}
              </p>
            </motion.div>
            ...
            >
              <Link
                ...
                className="inline-flex items-center gap-2 px-4 py-2 bg-muted rounded-full hover:bg-primary/10 hover:text-primary transition-all text-sm"
              >
            ...
            <EventFilters
              filters={filters}
              onFiltersChange={(newFilters) => setFilters({ ...newFilters, category: category.slug })}
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
              <div className="flex items-center gap-3">
                <h2 className="text-xl font-semibold text-foreground">
                  {hasSearched
                    ? events.length > 0
                      ? `${events.length} taller${events.length !== 1 ? "es" : ""} de ${category.shortName.toLowerCase()} en ${city.name}`
                      : `No hay talleres de ${category.shortName.toLowerCase()} en ${city.name}`
                    : `Buscando talleres de ${category.shortName.toLowerCase()}...`}
                </h2>
                  ...
                  {events.length === 0 && !isLoading ? (
                    <div className="text-center py-12 bg-card rounded-xl border">
                      ...
                      <h3 className="text-xl font-semibold text-foreground mb-2">
                        No encontramos talleres de {category.shortName.toLowerCase()}
                      </h3>
                      <p className="text-muted-foreground max-w-md mx-auto mb-4">
                        Prueba a ampliar el radio de búsqueda o explorar todas las categorías.
                      </p>
        ...
        <section className="py-12 bg-card border-t">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
            <h3 className="text-xl font-semibold text-foreground mb-6 text-center">
              {category.shortName} en otras ciudades
            </h3>
            <div className="flex flex-wrap justify-center gap-3">
              {["madrid", "barcelona", "valencia", "sevilla", "bilbao", "malaga", "zaragoza"]
                ...
                .map((citySlug) => (
                  <Link
                    key={citySlug}
                    href={`/eventos/${citySlug}/${category.slug}`}
                    className="px-4 py-2 bg-muted rounded-full hover:bg-primary/10 hover:text-primary transition-all text-sm"
                  >
